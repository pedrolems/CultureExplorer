...
export function Avatar({ initials, size = 64, colors = [L.blue, L.accent], style }) {
  return (
    <View style={[{
      width: size,
      height: size,
...
export function StarRow({ rating }) {
  return (
    <View style={{ flexDirection: 'row', gap: 2 }}>
      {[1, 2, 3, 4, 5].map(i => (
        <Text key={i} style={{ color: i <= rating ? L.star : L.hint, fontSize: 12 }}>★</Text>
      ))}
    </View>
  );
}

export function Tag({ label, color = L.accent, bg = L.accentSoft }) {
  return (
    <View style={{
      backgroundColor: bg,
      borderRadius: 20,
